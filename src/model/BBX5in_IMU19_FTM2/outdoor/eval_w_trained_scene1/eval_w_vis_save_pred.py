...
'''
Command example:
$ python3 eval_w_vis_save_pred.py -fps 10 -k 10 -tt rand_ss -wd_ls 1 1 1 0 -tsid_idx 0
$ python3 eval_w_vis_save_pred.py -fps 10 -k 10 -tt rand_ss -wd_ls 1 1 1 0 -tsid_idx 9
$ python3 eval_w_vis_save_pred.py -fps 10 -k 10 -tt rand_ss -wd_ls 1 1 1 0 -bw -tsid_idx 9
$ python3 eval_w_vis_save_pred.py -fps 10 -k 10 -tt crowded -wd_ls 1 1 1 0 -tsid_idx 9
$ python3 eval_w_vis_save_pred.py -fps 10 -k 10 -tt crowded -wd_ls 1 1 1 0 -bw -tsid_idx 9
'''
...
class Config:
    def __init__(self):
        ...
        self.test_type = self.args.test_type # 'random' # 'random', 'hard', 'rand_ss' # edit
        self.train_scene_id_idx_ls = self.args.train_scene_id_idx_ls # [0, 1, 2, 3, 4] # edit
        print(); print()
        print('self.train_scene_id_idx_ls: ', self.train_scene_id_idx_ls)
        if self.args.loss == 'mse': self.model_id = 'X22_outdoor_BBX5in_IMU19_FTM2_rand_ss_scene1' # 'X22_BBX5in_IMU19_FTM2_scene1' # edit
        elif self.args.loss == 'b': self.model_id = 'X22_outdoor_BBX5in_IMU19_FTM2_Bloss_scene1' # edit
        if self.test_type == 'rand_ss' or self.test_type == 'crowded': self.model_id += '_seq_' + str(self.args.test_seq_id_idx)
        self.seq_root_path = self.root_path + '/Data/datasets/RAN/seqs'
        self.seq_root_path_for_model = self.root_path + '/Data/datasets/RAN4model/seqs'
        self.exp_root_path = self.seq_root_path + '/exps'
        self.exp_id = 'exp0'
        self.exp_path = self.exp_root_path + '/' + self.exp_id
        ...
        self.train_test = 'train' # edit
        self.train_scene_id_ls = [1] # [0, 1, 2, 3, 4]# edit
        self.train_seq_id_ls, self.test_seq_id_ls = [], []

        self.test_seq_id_idx = self.args.test_seq_id_idx

        if self.test_type == 'crowded':
            self.test_seq_id_ls_path = self.exp_path_for_model + '/' + self.test_type + '_test_seq_id_ls.csv' # edit
        else:
            for train_scene_id_idx in self.train_scene_id_idx_ls:
                if self.test_type == 'normal':
                    self.train_seq_id_ls_path = self.exp_path_for_model + '/train_seq_id_ls_scene' + str(train_scene_id_idx) + '.csv'
                elif self.test_type == 'rand_ss' or self.test_type == 'crowded':
                    self.train_seq_id_ls_path = self.exp_path_for_model + '/rand_ss_seq_id_ls_scene' + str(train_scene_id_idx) + '.csv'
                with open(self.train_seq_id_ls_path, 'r') as f:
                    csv_reader = reader(f)
                    for i, row in enumerate(csv_reader):
                        # crowded '20211004_144536' in scene1 is hold out
                        if i != self.test_seq_id_idx and 'scene' not in row[0] and row[0] != '20211004_144536': self.train_seq_id_ls.append(row[0]) # e.g. 20211006_141820
            self.test_seq_id_ls_path = self.exp_path_for_model + '/' + self.test_type + '_seq_id_ls_scene1.csv' # edit

        # if self.test_type == 'normal':
        #     self.test_seq_id_ls_path = self.exp_path_for_model + '/' + self.test_type + '_test_seq_id_ls.csv'
        # if self.test_type == 'rand_ss':
        #     self.test_seq_id_ls_path = self.exp_path_for_model + '/' + self.test_type + '_seq_id_ls_scene'  + str(self.train_scene_id_idx_ls[0]) + '.csv' # edit
        # elif self.test_type == 'crowded': pass; # Will be updated in prepare_testing_data()

        print(); print() # debug
        print(self.train_seq_id_ls)
        '''
        e.g. ['20210907_143435', '20210907_144603', '20210907_145803', '20210907_150313',
            '20211004_142757', '20211004_143157', '20211004_143730', '20211004_144125',
            '20211006_141820', '20211006_142225', '20211006_142625', '20211006_143020',
            '20211007_104004', '20211007_103551', '20211007_103114', '20211007_102724',
            '20211007_133041', '20211007_133527', '20211007_133915', '20211007_143048']
        '''

        with open(self.test_seq_id_ls_path, 'r') as f:
            csv_reader = reader(f)
            for i, row in enumerate(csv_reader): self.test_seq_id_ls.append(row[0])
        print(); print() # debug
        print(self.test_seq_id_ls)
        '''
        e.g. ['20210907_145202', '20211004_142306', '20211006_152208', '20211007_105924',
            '20211007_134632']
        '''
        self.seq_id = '' # '20210907_143435' # dummy one, to be updated in prepare_testing_data()
        # if self.seq_id in self.train_seq_id_ls: self.train_test = 'train'
        # elif self.seq_id in self.test_seq_id_ls: self.train_test = 'test'
        # print(); print() # debug
        # print('self.train_test: ', self.train_test)
        ...
        self.text_start_row = 756 # 600

        # -------
        #  Color
        # -------
        self.color_ls = ['crimson', 'lime green', 'royal blue', 'chocolate', 'purple', 'lemon', \
            'golden rod', 'dark cyan', 'dodger blue', 'dark orchid', 'peru', 'lavender', 'honeydew',
            'sandy brown']
        self.color_dict = {
            'crimson': (60,20,220),
        ...
        if '2020122' in self.seq_id: self.scene_id = 0
        elif '20211004' in self.seq_id: self.scene_id = 1
        elif '20211006' in self.seq_id: self.scene_id = 2
        elif self.seq_id < '20211007_120000': self.scene_id = 3
        elif self.seq_id > '20211007_120000': self.scene_id = 4
        print(); print() # debug
        print('self.scene: ', self.scene_id)

        if self.scene_id == 0 or self.scene_id == 1: self.subjects = [43, 80, 0]
        ...
        if self.args.test_type == 'rand_ss':
            self.img_path = '../../../../../Data/datasets/RAN/seqs/outdoor/scene1/' + self.seq_id + '/' + self.img_type
        else:
            self.img_path = '../../../../../Data/datasets/RAN/seqs/outdoor/scene1/20211004_144536/' + self.img_type
        ...
        self.BBXC3_sync_dfv3_path = self.sync_dfv3_path + '/BBXC3_sync_dfv3.pkl'
        self.BBX5_sync_dfv3_path = self.sync_dfv3_path + '/BBX5_sync_dfv3.pkl'
        ...
        self.scene_eval_stats = {'gd': {'BBX': {'TP': 0, 'TN': 0, 'FP': 0, 'FN': 0, 'correct_num': 0, 'total_num': 0, \
                                        ...
                                        'BBXwDelta': {'TP': 0, 'TN': 0, 'FP': 0, 'FN': 0, 'correct_num': 0, 'total_num': 0, \
                                            'IDSWITCH': 0, 'ts16_dfv3_BBXwDelta_IDP': 0.0, 'cumu_BBXwDelta_IDP': 0.0}, \
...
def MyNet():
    ...
    def Bhatt_loss(y_true, y_pred):
        ...
        term0 = tf.math.truediv(tf.math.log(tf.math.add(tf.math.add(tf.math.truediv(\
                        tf.math.truediv(tf.math.pow(sig_true, 2) + small_num, tf.math.pow(sig_pred, 2) + small_num), 4.), \
                        tf.math.truediv(tf.math.pow(sig_pred, 2) + small_num, tf.math.pow(sig_true, 2) + small_num)), 2.) + small_num), 4.)
...
def vis_tracklet(img, seq_in_BBX5_, subj_i, Cam_ID, Phone_PRED, Phone_GND, curr_IDP, cumu_IDP):
    ...
    subj_color = C.color_dict[random.choice(C.color_ls)]
    ...
    for k_i in range(C.recent_K):
        if k_i < len(seq_in_BBX5_) - 1:
            ...
            if k_i == C.recent_K - 2:
                text = ''
                if isinstance(Phone_PRED, str) or Phone_PRED >= len(C.subjects):
                    text = Cam_ID + '-' + str(Phone_PRED)
                if isinstance(Phone_GND, str) or Phone_GND >= len(C.subjects):
                    text += ' GT: ' + str(Phone_GND)
                if not isinstance(Phone_PRED, str) and not isinstance(Phone_GND, str) \
                    and Phone_PRED < len(C.subjects) and Phone_GND < len(C.subjects):
                    # print('Phone_PRED: ', Phone_PRED)
                    # print('Phone_GND: ', Phone_GND)
...
def prepare_testing_data():
    ...
    if C.args.test_type == 'rand_ss': C.seq_id = C.test_seq_id_ls[C.test_seq_id_idx] # C.test_seq_id_ls[C.test_seq_id_idx] # edit
    elif C.args.test_type == 'crowded': C.seq_id = '20211004_144536' # edit

    print()
    if '20210907' in C.seq_id: C.scene_id = 0
    elif '20211004' in C.seq_id: C.scene_id = 1
    elif '20211006' in C.seq_id: C.scene_id = 2
    elif C.seq_id < '20211007_120000': C.scene_id = 3
    elif C.seq_id > '20211007_120000': C.scene_id = 4
    print(); print() # debug
    print('C.scene: ', C.scene_id)

    if C.scene_id == 0 or C.scene_id == 1: C.subjects = [43, 80, 0]
    else: C.subjects = [43, 80, 53, 0]
    print(); print() # debug
    print('C.subjects: ', C.subjects)
    ...
    if C.args.test_type == 'rand_ss':
        C.img_path = '../../../../../Data/datasets/RAN/seqs/outdoor/scene1/' + C.seq_id + '/' + C.img_type
    else:
        C.img_path = '../../../../../Data/datasets/RAN/seqs/outdoor/scene1/20211004_144536/' + C.img_type
    ...
    with open(C.RGB_ts16_dfv3_ls_path, 'r') as f:
        ...
        print(C.RGB_ts16_dfv3_ls_path, 'loaded!')
        print('C.RGB_ts16_dfv3_ls[:5]: ', C.RGB_ts16_dfv3_ls[:5])
    ...
    # ----------------
    C.BBX5_dim = 5
    C.BBX5_sync_dfv3_path = C.sync_dfv3_path + '/BBX5_sync_dfv3.pkl'
    ...
    C.video_len = len(C.RGB_ts16_dfv3_valid_ls) # len(C.ts16_dfv3_BBX5_all)
    print(); print() # debug
    ...
    assert np.shape(C.seq_in_BBX5)[2] == np.shape(C.seq_out_BBX5_Others)[2] == C.BBX5_dim
...
def eval_association():
    ...
    for win_i in range(C.n_wins):
        ...
        if ts16_dfv3 in C.seq_subj_i_in_view_dict.keys():
            ...
            if n > 0:
                ...
                for r_i, subj_i_r in enumerate(seq_subj_i_in_view_ls_):
                    ...
                    # -------------------------
                    for c_i in range(len(C.subjects) - 1):
                ...
                first_other_idx = len(C.subjects) - 1
                ...
                for i, col_i in enumerate(col_ind_Cam):
                    ...
                    if pred_id in range(len(C.subjects) - 1): # Predicted as Phone holders # Positive
                ...
                if C.vis:
                    for i, seq_subj_i in enumerate(seq_subj_i_in_view_ls_):
                        ...
                        # Others
                            ...
                            else: PRED = seq_subj_i_in_view_ls_[row_ind_Phone[i]] # col_ind_Phone[i]
        ...
        if C.save_vis:
            write_folder = './' + C.test_type + '/' + C.vis_type
